...
class AOATTS_CLF_MultiRate(gym.Env):
    """
    Autonomous Obstacle Avoidance and Target Tracking of UAV 
    with Multi-Rate CLF-QP Filter for Stability Guarantees
    
    RL Policy: 1 Hz (updates every 1.0 seconds)
    QP Filter: 10 Hz (updates every 0.1 seconds)
    Physics: 100 Hz (0.01 second timestep)
    """
    ...
    def __init__(self,
                 ...
                 use_clf_filter=True,
                 clf_filter=None,
                 rl_frequency=1.0,      # RL policy frequency (Hz)
                 qp_frequency=10.0,      # QP filter frequency (Hz)
                 physics_frequency=100.0 # Physics simulation frequency (Hz)
                 ):
        ...
        print(f"\n{'='*60}")
        print(f"MULTI-RATE CONTROL CONFIGURATION")
        print(f"{'='*60}")
        print(f"RL Policy Frequency:    {self.rl_frequency} Hz (dt = {self.rl_dt:.3f}s)")
        print(f"QP Filter Frequency:    {self.qp_frequency} Hz (dt = {self.qp_dt:.3f}s)")
        print(f"Physics Frequency:      {self.physics_frequency} Hz (dt = {self.physics_dt:.3f}s)")
        print(f"\nControl Hierarchy:")
        print(f"  • Physics steps per QP update:  {self.physics_steps_per_qp}")
        print(f"  • QP updates per RL decision:   {self.qp_steps_per_rl}")
        print(f"  • Total physics steps per RL:   {self.physics_steps_per_qp * self.qp_steps_per_rl}")
        print(f"{'='*60}\n")
        
        # Store last RL action
        self.last_rl_action = None
        
        # QP filter statistics
    ...
    def step(self,
        ...
        """
        Multi-rate control step:
        1. RL policy provides action (1 Hz)
        2. QP filter refines action 10 times (10 Hz) 
        3. Physics simulates 10 steps per QP update (100 Hz)
        """
        # Store the new RL action
        rl_action_preprocessed = self._preprocessAction(action)
        self.last_rl_action = rl_action_preprocessed
        
        # Execute multiple QP updates for this single RL decision
        for qp_step in range(self.qp_steps_per_rl):
            # Apply CLF-QP filter at 10 Hz
            if self.use_clf_filter and self.clf_filter is not None:
                state_dict = self._get_state_dict()
                filtered_action, clf_info = self.clf_filter.filter_action(
                    self.last_rl_action,
                    state_dict
                )
                ...
                self.total_action_deviation += clf_info['action_deviation']
            else:
                filtered_action = self.last_rl_action
                self.clf_info = None
            
            # Execute physics steps at 100 Hz
            for physics_step in range(self.physics_steps_per_qp):
                self._physics(filtered_action)
                p.stepSimulation(physicsClientId=self.CLIENT)
            
            # Update kinematic information after each QP cycle
            self._updateAndStoreKinematicInformation()
            
            # Check for early termination/truncation at QP frequency
            if self._computeTerminated() or self._computeTruncated():
                break
        
        # Store last distance and angle (after all QP updates)
        self.LAST_AGENT_TARGET_DIST = np.linalg.norm([
            self.agent_pos[0]-self.target_pos[0], 
            self.agent_pos[1]-self.target_pos[1]
        ])
        obs = self._computeObs()
        self.LAST_ALPHA = obs[17]
        self.prev_ray_dist = obs[6:15]

        # Store current distance
        self.AGENT_TARGET_DIST = np.linalg.norm([
            self.agent_pos[0]-self.target_pos[0], 
            self.agent_pos[1]-self.target_pos[1]
        ])
        ...
        # Advance the step counter (RL steps)
        self.step_counter = self.step_counter + 1
        return obs, reward, terminated, truncated, info

    def close(self):
        if self.qp_call_count > 0:
            print(f"\n{'='*60}")
            print(f"QP FILTER STATISTICS (Multi-Rate)")
            print(f"{'='*60}")
            print(f"Total QP calls:         {self.qp_call_count}")
            print(f"QP success rate:        {self.qp_success_count/self.qp_call_count*100:.2f}%")
            print(f"Avg action deviation:   {self.total_action_deviation/self.qp_call_count:.4f}")
            print(f"{'='*60}\n")
    ...
    def _get_state_dict(self):
        """Get state dictionary for CLF-QP filter"""
        return {
            'agent_pos': np.array(self.agent_pos),
            'agent_vel': np.array(self.agent_vel),
            'target_pos': np.array(self.target_pos),
            'target_vel': np.array(self.target_vel)
        }
    ...
    def _computeInfo(self, terminated=False, truncated=False):
        ...
        info = {
            ...
            "qp_calls_this_episode": self.qp_call_count,
            "qp_success_rate": self.qp_success_count / self.qp_call_count if self.qp_call_count > 0 else 0,
            "avg_action_deviation": self.total_action_deviation / self.qp_call_count if self.qp_call_count > 0 else 0
        }
        
        # Add latest CLF info if available
        if self.clf_info is not None:
            info['clf_info'] = self.clf_info
