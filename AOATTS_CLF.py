...
class AOATTS_CLF(gym.Env):
    """
    Autonomous Obstacle Avoidance and Target Tracking of UAV 
    with CLF-QP Filter for Stability Guarantees
    """
    ...
    def step(self,
        ...
        # Apply CLF-QP filter if enabled
        if self.use_clf_filter and self.clf_filter is not None:
            state_dict = self._get_state_dict()
            clipped_action, self.clf_info = self.clf_filter.filter_action(
                clipped_action,
                state_dict
            )
