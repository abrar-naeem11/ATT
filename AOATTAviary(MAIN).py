...
class AOATTAviary(gym.Env):
    ...
    def _housekeeping(self):
        ...
        self.DRONE_IDS = np.array([p.loadURDF(os.path.join("assets", self.URDF),
                                              self.INIT_XYZS[i, :],
    ...
    def _actionSpace(self):
        ...
        if self.ACT_TYPE in [ActionType.RPM, ActionType.VEL]:
            ...
            print(f"[DEBUG] ACT_TYPE: {self.ACT_TYPE}, type: {type(self.ACT_TYPE)}")
...
def _drawGrid(self, spacing=25, min_coord=-50, max_coord=250):
    """
    Draws a grid on the XY plane from min_coord to max_coord with given spacing.
    ...
    min_coord : int
        Start of the grid (in both x and y).
    max_coord : int
        End of the grid (in both x and y).
    """
    color = [0.6, 0.6, 0.6]  # Light gray grid lines
    line_width = 0.3

    # Vertical lines (constant x, varying y)
    for x in range(min_coord, max_coord + 1, spacing):
        p.addUserDebugLine(
            [x, min_coord, 0],
            [x, max_coord, 0],
            color,
    ...
    for y in range(min_coord, max_coord + 1, spacing):
        p.addUserDebugLine(
            [min_coord, y, 0],
            [max_coord, y, 0],
            color,
